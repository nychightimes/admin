# Google Cloud Run Deployment Guide - Admin Project

This guide provides step-by-step instructions for deploying the Next.js admin application to Google Cloud Run with automated GitHub CI/CD.
...
Before deploying, ensure you have all environment variables ready. Common variables for the admin application include:
...
- API keys for third-party services (email, etc.)
...
gcloud builds triggers create github \
  --name=admin-deploy \
  --repo-name=YOUR_ADMIN_REPO_NAME \
...
cd /Users/musaver/Desktop/central-distros-105th/username-password/admin
...
```bash
git add .
git commit -m "Deploy admin to Cloud Run"
...
```bash
gcloud run services describe admin --region=us-central1 --format="value(status.url)"
```
...
gcloud run services update admin \
  --region=us-central1 \
  --update-env-vars="NEXTAUTH_URL=https://your-admin-service-url.run.app"
```
...
**View Cloud Run Logs:**
```bash
gcloud run services logs read admin --region=us-central1 --limit=50
```
...
- If using Vercel Blob Storage, ensure `BLOB_READ_WRITE_TOKEN` is set
...
# Delete a service
gcloud run services delete admin --region=us-central1

# View service configuration
gcloud run services describe admin --region=us-central1

# Update service configuration
gcloud run services update admin --region=us-central1 [OPTIONS]

# View logs in real-time
gcloud run services logs tail admin --region=us-central1

# Rollback to previous revision
gcloud run services update-traffic admin --region=us-central1 --to-revisions=REVISION_NAME=100
```
